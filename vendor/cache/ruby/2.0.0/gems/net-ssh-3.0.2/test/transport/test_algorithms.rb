  ...
  class TestAlgorithms < Test::Unit::TestCase
    ...
    def test_allowed_packets
      (0..255).each do |type|
        ...
        case type
          when 1..4, 6..19, 21..49 then
            assert(Net::SSH::Transport::Algorithms.allowed_packet?(packet), "#{type} should be allowed during key exchange")
          else
            assert(!Net::SSH::Transport::Algorithms.allowed_packet?(packet), "#{type} should not be allowed during key exchange")
        end
      end
    end
    ...
    def test_allow_when_pending_should_be_true_only_for_packets_valid_during_key_exchange
      ...
      (0..255).each do |type|
        ...
        case type
          when 1..4, 6..19, 21..49 then
            assert(algorithms.allow?(packet), "#{type} should be allowed during key exchange")
          else
            assert(!algorithms.allow?(packet), "#{type} should not be allowed during key exchange")
        end
      end
    end
