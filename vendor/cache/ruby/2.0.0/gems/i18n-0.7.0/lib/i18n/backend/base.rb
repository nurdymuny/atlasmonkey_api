      ...
      def localize(locale, object, format = :default, options = {})
        ...
        format = format.to_s.gsub(/%[aAbBpP]/) do |match|
          case match
            ...
            when '%p' then
              I18n.t(:"time.#{object.hour < 12 ? :am : :pm}", :locale => locale, :format => format).upcase if object.respond_to? :hour
            when '%P' then
              I18n.t(:"time.#{object.hour < 12 ? :am : :pm}", :locale => locale, :format => format).downcase if object.respond_to? :hour
          end
        end
