...
class RDoc::Context < RDoc::CodeObject
  ...
  def add_method method
    ...
    if known then
      if @store then # otherwise we are loading
        ...
        previously = ", previously in #{known.file}" unless method.file == known.file
  ...
  def each_section # :yields: section, constants, attributes
    ...
    constants = @constants.group_by do |constant|
      constant.section
    end
    attributes = @attributes.group_by do |attribute|
      attribute.section
    end
  ...
  def section_contents
    ...
    return [] if sections.length == 1 and not sections.first.title
