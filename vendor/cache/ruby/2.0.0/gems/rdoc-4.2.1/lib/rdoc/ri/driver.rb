...
'PAGER' environment variable.
  ...
  def class_document_constants out, klass # :nodoc:
    ...
    list.items.concat constants.map { |constant|
                        ...
                        parts << RDoc::Markup::Paragraph.new('[not documented]') if parts.empty?
  ...
  def complete_klass name, klass, selector, method, completions # :nodoc:
    ...
    if name !~ /#|\./ then
      ...
      completions.concat klasses.grep(/^#{Regexp.escape name}[^:]*$/) if name =~ /::$/
  ...
  def find_store name
    @stores.each do |store|
      ...
      return source if store.type == :gem and source =~ /^#{Regexp.escape name}-\d/
    end
  ...
  def load_methods_matching name
    ...
    find_methods name do |store, klass, ancestor, types, method|
      ...
      methods << load_method(store, :class_methods, ancestor, '::', method) if [:class, :both].include? types

      methods << load_method(store, :instance_methods, ancestor, '#', method) if [:instance, :both].include? types

      found << [store, methods.compact]
    end

    found.reject do |path, methods|
      methods.empty?
    end
  end
  ...
  def render_method_superclass out, method # :nodoc:
    return unless method.respond_to?(:superclass_method) and method.superclass_method
